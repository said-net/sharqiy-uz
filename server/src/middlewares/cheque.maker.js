...
module.exports = async (o) => {
    ...
    doc.text(`ID: ${o?.id}${c}Mijoz: ${o?.name}${c}Raqam: ${o?.phone}${c}Maxsulot: ${o?.title}${c}Miqdori: ${o?.count} ta${c}Bonus: +${o?.bonus} ta${c}Narxi: ${Number(o?.price).toLocaleString()} so'm${c}Buyurtma sanasi: ${o?.date}${c}Manzil: ${Regions?.find(e => e.id === o?.region).name} - ${Cities?.find(e => e.id === o?.city).name}${c}Izoh: ${o?.about}${c}Operator: ${o?.operator_name} | ${o?.operator_phone}${c}Veb sayt: sharqiy.uz`);

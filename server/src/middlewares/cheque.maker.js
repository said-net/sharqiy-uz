...
module.exports = async (o) => {
    ...
    doc.font('sans').fontSize(10).text(`ID: ${o?.id}${c}Mijoz: ${o?.name}${c}Raqam: ${o?.phone}${c}Maxsulot: ${o?.title}${c}Miqdori: ${o?.count} ta${c}Bonus: +${o?.bonus} ta${c}Narxi: ${Number(o?.price).toLocaleString()} so'm${c}Buyurtma sanasi: ${o?.date}${c}Manzil: ${Regions?.find(e => e.id === o?.region)?.name} - ${Cities?.find(e => e.id === o?.city)?.name}${c}Izoh: ${o?.about}${c}Operator: ${o?.operator_phone} | ${o.operator_name}${c}Veb sayt: sharqiy.uz${c}Call markaz: +998339306464`);
